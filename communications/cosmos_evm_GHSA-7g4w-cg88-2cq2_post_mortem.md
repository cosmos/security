...
The initial bug bounty report received on April 25th containing the underflow issue included a proof of concept demonstrating theft of funds on several untagged versions of Cosmos EVM. After receiving the report, the Cosmos Labs team made attempts to reproduce the fund-theft on the supported versions of Cosmos EVM at the time (v0.6.0, v0.6.1, v0.5.0, and v0.5.1), testing multiple network configurations using our standard vulnerability testing process. We were unable to reproduce the fund-theft vulnerability on supported versions using 18-decimal configurations. Based on those results, we incorrectly concluded that the vulnerability could only result in theft of funds on non-18 decimal networks in production. Because all known production Cosmos EVM networks used 18-decimal configurations, we assessed that the vulnerability did not pose a risk to user funds on live networks. 
